...
// Sustituye a createClassCheckout. Mantiene lo que ya estaba bien (el importe sale de la
// reserva y se contrasta con las tarifas del profesor; los metadatos llevan booking_id y
// bookingId) y arregla el reparto del dinero:
//
// Antes, con Stripe conectado, el 100 % iba al profesor fuera cual fuera su plan. Un profesor
// del plan sin cuota cobraba integro y a Menttio le tocaba reclamarle el 10 % a mano. Ahora
// se usa application_fee_amount: Stripe parte el cobro en el origen, el profesor recibe su
// parte y la comision se queda en Menttio sin que nadie tenga que transferir nada.
//
// La comision de Stripe la asume el profesor en los dos casos: se descuenta de lo que recibe.
...
export default async function(req) {
  try {
    ...
    console.error('Error en classCheckout:', error);
