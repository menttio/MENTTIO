...
const SYSTEM_PROMPT = `Eres el asistente de ventas y soporte de Menttio, una plataforma todo en uno para profesores particulares.

KNOWLEDGE BASE:
Menttio es una plataforma todo en uno para profesores particulares. El profesor solo configura materias, precio y disponibilidad — Menttio gestiona el resto automáticamente.

FUNCIONALIDADES:
- Agenda inteligente: define disponibilidad una vez, los alumnos reservan solos en los huecos libres, sin solapamientos ni WhatsApps.
...
- Materiales accesibles: el profesor sube apuntes una vez, los alumnos los encuentran siempre organizados.
- Gestión de alumnos: historial, seguimiento y progreso en un solo lugar.
- Chat con alumnos integrado.
...
MENTTIO NO: no interviene en el contenido de las clases, no fija precios, no cobra comisión, no controla los horarios.
...
- Si el usuario muestra interés claro, dale el link de registro: https://menttio.com/TeacherSignup
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        ...
        role: 'assistant',
        content: typeof response === 'string' ? response : (response?.text || 'Lo siento, algo falló. ¿Puedes repetirlo?'),
      }]);
    } catch {
      setMessages(prev => [...prev, {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: 'Ups, algo falló por mi parte. ¡Prueba de nuevo!',
      }]);
    } finally {
      setIsTyping(false);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end gap-3">
      ...
      <motion.button
        ...
        className="w-14 h-14 rounded-full bg-[#41f2c0] hover:bg-[#35d4a7] shadow-lg hover:shadow-xl transition-all flex items-center justify-center text-white"
        style={{ boxShadow: '0 4px 20px rgba(65, 242, 192, 0.4)' }}
      >
