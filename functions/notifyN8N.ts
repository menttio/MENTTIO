...
Deno.serve(async (req) => {
  try {
    ...
    // Obtener URL del webhook desde variables de entorno según el tipo de evento
    ...
    if (!webhookUrl) {
      console.error(`${isCancellation ? 'N8N_CANCEL_WEBHOOK_URL' : 'N8N_WEBHOOK_URL'} no está configurado`);
